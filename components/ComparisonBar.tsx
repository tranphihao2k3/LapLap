...
export default function ComparisonBar() {
  ...
  return (
    <div className="fixed bottom-16 md:bottom-0 left-0 right-0 z-[110] mb-1 md:mb-0 pointer-events-none flex flex-col items-center">
      ...
      <div
        className="pointer-events-auto bg-blue-600 text-white px-6 py-2 rounded-t-xl shadow-lg cursor-pointer flex items-center gap-2 hover:bg-blue-700 transition-colors"
        onClick={toggleOpen}
      >
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div className="flex-1 w-full grid grid-cols-2 md:grid-cols-4 gap-4">
                  {selectedProducts.map((product) => (
                    <div
                      key={product._id}
                      className="relative group bg-gray-50 rounded-lg p-2 border border-gray-200 flex items-center gap-3"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4
                          className="text-xs font-semibold text-gray-800 line-clamp-2 leading-tight"
                          title={product.name}
                        >
                          {product.name}
                        </h4>
                  ...
                  {Array.from({ length: 4 - selectedProducts.length }).map(
                    (_, i) => (
                      <div
                        key={`empty-${i}`}
                        className="hidden md:flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg h-[74px] bg-gray-50/50 text-gray-400 text-xs"
                      >
                        Thêm sản phẩm
                      </div>
                    ),
                ...
                <div className="flex items-center gap-3 w-full md:w-auto mt-2 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100">
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg font-bold shadow-lg transition-all whitespace-nowrap ${
