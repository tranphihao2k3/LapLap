...
export default function BuybackOrdersPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        showToast(
          editingOrder ? "Cập nhật thành công!" : "Tạo đơn thu cũ thành công!",
        );
        handleCloseModal();
        fetchOrders();
      } else {
  ...
  const filteredOrders = orders.filter((o) => {
    const matchesSearch =
      o.buybackNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSaveQuotedPrice = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/buyback-orders/${selectedOrder._id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (data.success) {
        showToast("Đã lưu giá báo thành công!");
        setSelectedOrder(data.data);
        fetchOrders();
      } else {
        showToast(data.error || "Không thể lưu giá báo", "error");
      }
    } catch (error) {
      showToast("Lỗi kết nối khi lưu giá báo", "error");
    }
  };

  const openZaloQuote = () => {
    if (!selectedOrder?.sellerPhone) {
      showToast("Không có số điện thoại người bán", "error");
      return;
    }

    const quoted = Number(quotedPriceInput || 0);
    if (!quoted || quoted <= 0) {
      showToast("Vui lòng nhập giá báo trước khi gửi Zalo", "error");
      return;
    }

    const message = `LapLap xin báo giá máy ${selectedOrder.productInfo?.model || ""}: ${formatPrice(quoted)}. Nếu đồng ý, bạn phản hồi giúp shop để chốt đơn nhé.`;
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-xs font-black text-slate-500 uppercase tracking-wider">
                    Trạng thái
                  </th>
                  <th className="px-4 py-3 text-center text-xs font-black text-slate-500 uppercase tracking-wider">
                    Thao tác
                    ...
                    <tr key={order._id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="text-sm text-slate-800">
                          {order.productInfo.brand} {order.productInfo.model}
                        </div>
                        <div className="text-xs text-slate-500">
                          {order.productInfo.condition}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-black uppercase tracking-wider ${STATUS_LABELS[order.status].color}`}
                        >
                          <StatusIcon size={12} />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-y-auto animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300">
            ...
            <div className="p-6 space-y-6">
              {/* Status */}
              <div
                className={`p-4 rounded-xl ${STATUS_LABELS[selectedOrder.status].color} bg-opacity-20`}
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold">
                    Trạng thái: {STATUS_LABELS[selectedOrder.status].label}
                  </span>
                  <div className="flex gap-2">
                    {selectedOrder.status === "pending" && (
                      <>
                        <button
                          onClick={() =>
                            handleUpdateStatus(selectedOrder._id, "inspecting")
                          }
              ...
              <div className="space-y-2">
                ...
                <div className="bg-slate-50 p-4 rounded-xl space-y-2">
                  <p>
                    ...
                    {selectedOrder.productInfo.brand}
                  </p>
                  <p>
                    <span className="text-slate-500">Model:</span>{" "}
                    <span className="font-bold">
                      {selectedOrder.productInfo.model}
                    </span>
                  </p>
                  {selectedOrder.productInfo.serialNumber && (
                    <p>
                  ...
                  <p>
                    ...
                    {selectedOrder.productInfo.condition}
                  </p>
                </div>
                      ...
                      {selectedOrder.images.map((img, i) => (
                        <PhotoView key={i} src={img}>
                          <img
                            ...
                            className="aspect-square object-cover rounded-lg cursor-zoom-in"
                          />
                        </PhotoView>
                      ))}
                    </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl space-y-2">
                  <p>
                    ...
                    <span className="text-xl font-black text-emerald-600">
                      {formatPrice(selectedOrder.buyPrice)}
                    </span>
                  </p>
                  <p>
                ...
                <div className="bg-slate-50 p-4 rounded-xl space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold inline-flex items-center justify-center gap-2"
                    >
